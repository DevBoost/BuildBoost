...
public class TextResourcePluginGenerator {
	...
	public void run(
			...
			String projectName, 
			String buildDirPath, 
			List<String> pluginPaths) throws Exception {
		...
		Set<EObject> unresolvedProxies = CsResourceUtil.findUnresolvedProxies(rs);
		...
		BuildBoostGenerationContext context = new BuildBoostGenerationContext(
				folderConnector, 
				new BuildBoostProblemCollector(), 
				syntax, 
				rootFolder, 
				pathToCsFile,
				projectName);
		Result result = new BuildBoostGenerator().run(
				context, 
				new BuildBoostLogMarker(), 
				new BuildBoostProgressMonitor()
		);
		if (result != Result.SUCCESS) {
			if (result == Result.ERROR_FOUND_UNRESOLVED_PROXIES) {
				for (EObject unresolvedProxy : result.getUnresolvedProxies()) {
					System.out.println("Found unresolved proxy \"" + ((InternalEObject) unresolvedProxy).eProxyURI() + "\" in " + unresolvedProxy.eResource());
				}
	...
	private void registerFactoriesAndPackages() {
		...
		// TODO this is a MODIFIED copy of this method from class HeadlessCodeGenerator
		Map<String, Object> extensionToFactoryMap = Resource.Factory.Registry.INSTANCE.getExtensionToFactoryMap();
		extensionToFactoryMap.put("ecore", new EcoreResourceFactoryImpl());
		extensionToFactoryMap.put("genmodel", new EcoreResourceFactoryImpl());
		extensionToFactoryMap.put(new CsMetaInformation().getSyntaxName(), new CsResourceFactory());
